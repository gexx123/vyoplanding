// Lightweight Code128 Barcode SVG Generator
export function generateCode128Svg(text: string): string {
  if (!text) text = "890123456789";

  // Code128 pattern definitions (107 patterns, 11 modules wide each)
  ...
  // Code128-B mapping for ASCII 32 to 126
  const codeBStart = 104;
  let checksum = codeBStart;
  const codes: number[] = [codeBStart];
  ...
  for (let i = 0; i < bars.length; i++) {
    const width = parseInt(bars[i], 10) * 2;
    if (i % 2 === 0) {
      rects += `<rect x="${x}" y="10" width="${width}" height="${height}" fill="#000000" />`;
    }
    x += width;
  }
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalWidth} ${totalHeight}" width="100%" height="100%">
    ...
    ${rects}
    <text x="${totalWidth / 2}" y="${height + 30}" font-family="monospace" font-size="14" font-weight="bold" text-anchor="middle" fill="#000000">${text}</text>
  </svg>`;
}
